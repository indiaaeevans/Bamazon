...
// establish connection to the database--if successful, 
// run our first function to show the items available and start an order
  ...
  // check if valid numbers were submitted by the user
    ...
    // confirm what they wanted to order
      ...
      // check if your store has enough of the product to meet the customer's request.
          ...
          connection.query(queryUpdate, [res[0].stock_quantity - answers.quantity, answers.id], 
...
// ask if they would like to start a new order--if yes, then prompt them with the items again
function orderAgain() {
	inquirer.prompt([
    { name: "order",
      type: "confirm",
